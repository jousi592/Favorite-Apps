...
const Create = (ObjectOfTags = {}, ArrayOfProperties) => {
  ...
  for(key in object){
    ...
    children.forEach((child) => {
      if(child.includes("-c:")){
        ...
        let className = child.substr(child.indexOf(":") + 1);
        let getElement = child.replace(`-c:${className}`, "");
        // Push class to array of Classes
        this.newElements.push(getElement);
         if(className.includes(" ")){
          let classesArray = className.split(" ");
          [...classesArray].map(className => arraysOfElements.classes.push(className));
        }
        else {
          arraysOfElements.classes.push(className)
        }
      ...
      else if(child.includes("-d:")){
        // Get ID from array element
        let idName = child.substr(child.indexOf(":") + 1);
        let getElement = child.replace(`-d:${idName}`, "");
        this.newElements.push(getElement);
        // Push id to array of IDs
        arraysOfElements.ids.push(idName);
        let childElement = document.createElement(`${getElement}`);
 ...
 function insertProperties (arrayOfElements, objectOfProperties) {
   ...
   keyNames.forEach((nameOf) => {
     ...
     else if(checkArray(this.htmlTags, nameOf)){
       getElement = document.querySelector(`${nameOf}`);
     }
     else if(checkArray(arraysOfElements.ids, nameOf)){
       getElement = document.querySelector(`#${nameOf}`);
     }
     else if(checkArray(arraysOfElements.classes, nameOf)){
       getElement = document.querySelectorAll(`.${nameOf}`);
       isClass = true;
     }
     // If you want to choose a perticular element by class name
     else if(nameOf.includes(":")){
       let index=  nameOf.substr(0, nameOf.indexOf(':'));
       let name = nameOf.substr(nameOf.indexOf(":") + 1);
 
       // Check if "name" is a class, if not, the its a tag name
       if(checkArray(arraysOfElements.classes, name)){
     ...
     /* Set the object of properties for each element:
       ...
       */
     let properties = objectOfProperties[nameOf];
     ...
     for(property in properties) {
       ...
       if (animate){
         ...
         if(property == "name"){
           ...
           this.animationExtensions.forEach((prop) => {
             for(style in properties[property]){
               for(key in properties[property][style]){
                 let line = `${style}{${prop}${key}:${properties[property][style][key]};}`
           ...
           */
           ...
           if(isClass){
             if(getElement[0].className.length > 0){
               ...
               if(checkArray(arraysOfElements.styledElements, ClassName) != true){
               ...
               arraysOfElements.styledElements.push(ClassName);
             }
               ...
               // Check if class name hasent been styled already
               if(checkArray(arraysOfElements.styledElements, ID) != true){
               ...
               if(checkArray(arraysOfElements.styledElements, tag) != true){
               ...
               arraysOfElements.styledElements.push(tag);
             }
           }
           else {
             if(getElement.className.length > 0){
               ...
               // Mark the class name as styled
               arraysOfElements.styledElements.push(ID);
             }
             else if(getElement.className.length == 0){
               if(property.includes(":")){
                 let stylingBegins = `${tag}${property}{ ${getAllStyles} }`;
         ...
         // Check if property is innerHTML
           ...
           if(typeof properties[property] === "string"){
             ...
             if(checkArray(this.htmlTags, nameOf)){
               getParent = document.querySelector(`${nameOf}`);
             }
             else if(checkArray(arraysOfElements.ids, nameOf)){
               getParent = document.querySelector(`#${nameOf}`);
             }
               ...
               throw `: Your element "${nameOf}" could not be found`;
             }
           ...
           else if(getElement.classList.length != 0 || nameOf.includes(":")){
             getElement.setAttribute(`${property}`, `${properties[property]}`);
           }
           else {
             let tags = document.querySelectorAll(`${getElement.tagName}`);
             [...tags].map(element => element.setAttribute(`${property}`, `${properties[property]}`))
           }
         }
       }
       else {
         if (property == "style" || property.includes(":") && !property.includes("@")) {
           ...
           */
           ...
           let getAllStyles = arraOfStyles.join(" ");
           if(property.includes(":")){
             let stylingBegins = `.${nameOf}${property}{ ${getAllStyles} }`;
             let styleTag = document.querySelector("style");
             styleTag.insertAdjacentHTML('afterbegin', stylingBegins);
           }
           else {
             let stylingBegins = `.${nameOf}{ ${getAllStyles} }`;
             let styleTag = document.querySelector("style");
             styleTag.insertAdjacentHTML('afterbegin', stylingBegins);
           }
         }
         else {
           throw `: You can not assign "${property}", only style or ":" styling such as :hover,:after,:before can be assigned to "${nameOf}", which doesnt have a DOM element assinged to it.`;
         }
       }
     }
   })
